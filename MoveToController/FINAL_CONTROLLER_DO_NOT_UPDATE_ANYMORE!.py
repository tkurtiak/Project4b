...
pub_moveto= rospy.Publisher('/moveto_cmd_body',Point,queue_size=1)
...
def updatecenter(msg):
    ...
    global_marker_center[0]= msg.x - framex*0.5 # in camera center coords. pos right
    global_marker_center[1]= -msg.y + framey*0.5 # in cemera center coords.  pos up
    rospy.loginfo(msg)
    print(msg)
...
def trackwall():
    ...
    while isfirststep==True:
        wall_x = global_marker_center[0].copy()
        wall_y = global_marker_center[1].copy()

        if wall_x != 0 and wall_y != 0:

            # Determine if the wall is Tall or short
            # wallheight_thresh = 75 # percent of the image height above or below the centrer to consider the wall short or tall
            # # If the wall is below us when we are high, the wall must be short
            # if (wall_y < -wallheight_thresh and global_pos.position.z>1) or (wall_y < wallheight_thresh and global_pos.position.z<1):
            #     # Wall is short
            #     Z_cmd = 2.5 # set z height to 2 meters
            #     print('It looks like the wall is SHORT')
            # # If the wall is above us when we are low, the wall must be tall
            # elif (wall_y  > wallheight_thresh and global_pos.position.z<1) or (wall_y  > -wallheight_thresh and global_pos.position.z>1):
            #     # wall is tall
            #     Z_cmd = 0.7 # set z height to 0.5 meters
            ...
            errorx = wall_x
            print('Pixel Y error:', errorx)
            print('Angle to Wall:',phi_angle_to_wall*180/3.14)
            x_cmd = 0 #step*np.cos(phi_angle_to_wall)
            y_cmd = -.002*wall_x#-step*np.sin(phi_angle_to_wall) # positive movement is left, so we want to move right if angle is positive
            ...
            # IF WE ARE CLOSE, EXECUTE A SERIES OF COMMANDS TO GO OVER THE WALL AND FWD
            #if 75% of the contour is showing
            #   x_cmd = 1.5
            #   Z_cmd = 2
            #   y_cmd = 0
...
def moveto_body(x,y,z):
    ...
    if islanded==False:
        ...
        while error>.13 and islanded==False:# and np.linalg.norm(np.array([global_vel.linear.x, global_vel.linear.y, global_vel.linear.y]))>.1:
            print('error-------------------------------------------------------------------------')
            print(error)
            ...
            move_array[0]=.08*move_vect_body[0] - .23*velocity_vect_body[0] + .001*error_integral[0] #TUNE THIS
            move_array[1]=.08*move_vect_body[1] - .23*velocity_vect_body[1] + .001*error_integral[1]
            move_array[2]=.56*move_vect_body[2] - .10*velocity_vect_body[2] + .001*error_integral[2]
